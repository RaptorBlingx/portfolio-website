...
graph TB
  subgraph "Presentation Layer"
    VOICE[OVOS Voice Assistant] --> QD
    WEB[React + TypeScript Dashboard] --> API[FastAPI Backend]
  end
  subgraph "Intelligence Layer"
    QD[QueryDispatcher — 5 Query Types]
    QD --> KPI[KPI Retrieval]
    QD --> COMP[Comparison]
    QD --> TREND[Trend Analysis]
    QD --> ANOM[Anomaly Detection]
    QD --> WHAT[What-If Simulation]
    DOC[DocuBoT — RAG Knowledge] --> API
    PREV[PREVENTION — Predictive Maintenance] --> API
  end
  subgraph "Domain Layer — Immutable Models"
    API --> DOMAIN[Canonical Metrics Framework]
    DOMAIN --> ADAPTERS{Platform Adapters}
  end
  subgraph "Infrastructure Layer"
    ADAPTERS --> REN[RENERYO Adapter]
    ADAPTERS --> ES[EnergySuite Adapter]
    ADAPTERS --> GM[GreenMetrics Adapter]
    ADAPTERS --> MOCK[Mock Adapter — Zero-Config]
    DB[(TimescaleDB)] --> API
    MQTT[MQTT / Modbus TCP] --> SENSORS[IoT Sensors]
  end
  subgraph "Security & Ops"
    NGINX[Nginx TLS Reverse Proxy]
    GDPR[GDPR Audit Trails + RBAC]
...
| **DEC-002** | Universal Metric Framework — canonical names mapped per platform |
| **DEC-003** | Clean Architecture — strict layer separation, domain never imports infrastructure |
| **DEC-004** | Immutable Domain Models — frozen dataclasses, thread-safe |
| **DEC-005** | Zero-Config First Run — MockAdapter fallback, `docker compose up` → working system |
| **DEC-006** | Settings Service Pattern — no hardcoded credentials anywhere |
| **DEC-007** | Intelligence in Orchestration — adapters are dumb data fetchers; intelligence lives in QueryDispatcher, DocuBoT, PREVENTION |
