...
graph TB
  subgraph "Windows Desktop"
    BUBBLE[Electron Floating Bubble] -->|Global Hotkey| SEARCH[Instant Search]
    BUBBLE --> VARS[Variable Fill Dialog]
    BUBBLE --> COPY[One-Click Copy]
    BUBBLE -->|HTTP API| SERVER
  end
  subgraph "Web Dashboard"
    REACT[React 18 + TypeScript + Vite] --> CMD[Command Palette — Ctrl+K]
    REACT --> FOLDERS[Folder Organization]
    REACT --> VER[Version History — Last 5]
    REACT --> SMART[Smart Variables]
    REACT -->|REST API| SERVER
  end
  subgraph "Server — Ubuntu"
    SERVER[Express.js API — 11+ Endpoints]
    SERVER --> DB[(SQLite — Offline-First)]
    SERVER -->|Optimize| GEMINI[Google Gemini API]
  end
  subgraph "Deployment"
    DOCKER[Docker Multi-Stage Build]
    DOCKER --> NGINX[Nginx Reverse Proxy]
    DOCKER --> SUPER[Supervisord]
  end
`} />
