...
graph TB
  subgraph "User Interfaces"
    ...
    CHAT[RASA Chatbot]
    WEB[Web Dashboard — Analytics + KPIs]
    GRAFANA[10 SOTA Grafana Dashboards]
  end
  subgraph "Intelligence Layer"
    API[FastAPI Analytics Backend]
    ML[ML Engine — ARIMA/Prophet]
    ANOMALY[Anomaly Detection]
    SANKEY[Sankey Energy Flow Engine]
    COMPARE[Machine Comparison Analytics]
    LLM[LLM Service — Report Generation]
  end
  subgraph "Data Pipeline"
    NR[Node-RED — MQTT→Parse→Route→Process]
    SIM[Factory Simulator API]
    MQTT[MQTT Broker]
  end
  subgraph "Data Layer"
    DB[(TimescaleDB — 19.2M Readings)]
    MODELS[138 Baseline Models]
  end
  subgraph "IoT Layer"
    PLUGS[Smart Plugs]
