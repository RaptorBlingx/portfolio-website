...
graph TB
  subgraph "Hardware Layer"
    ESP[ESP32 Sensor Hubs — C++ Firmware] -->|MQTT| MOSQ[Mosquitto Broker]
    SHELLY[30+ Shelly Smart Plugs] -->|MQTT| MOSQ
    RPI[Raspberry Pi 4 + OctoPrint] -->|API| SIMPLY[SimplyPrint Cloud]
  end
  subgraph "Data Pipeline — Node-RED"
    MOSQ --> NR[6+ Processing Flows]
    NR --> INGEST[Ingest Shelly MQTT]
    NR --> TRAIN[Model Training]
    NR --> PREDICT[Live Predictor]
    NR --> DPP_FLOW[DPP Generation]
    NR --> ENV[Fetch Environment]
  end
  subgraph "Backend Services"
    NR --> DB[(PostgreSQL + TimescaleDB)]
    FLASK[Python Flask API] --> DB
    FLASK --> DPP_PDF[DPP PDF Reports]
    ML[ML Worker — Predictive Analytics] --> DB
    ANALYSIS[Interactive Analysis Engine] --> DB
  end
  subgraph "Visualization & Frontend"
    DB --> GRAFANA[Grafana — Pre-Provisioned Dashboards]
    NGINX[Nginx — Reverse Proxy + Frontend] --> GRAFANA
    NGINX --> FLASK
    NGINX --> CRUD[Device Management CRUD UI]
  end
`} />
